...
def connect_controller():
    ...
    for port in range(1, 10):
        try:
            ser = serial.Serial('COM%d' % port, 9600,
...
while True:
    ...
    halfImg = cv2.resize(image, (int(3264 / 3), int(2448 / 3)),
                         interpolation=cv2.INTER_AREA)
    cv2.putText(halfImg, "BC-SCANNER %s" % role, (10, 30),
                cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 255, 255), 1, cv2.LINE_AA)
    if mm is not None:
        cv2.putText(halfImg, "%dmm" % mm, (10, 60),
    ...
    if stage == 3:
        ...
        os.makedirs(DATAPATH + fn)
        logger.warning("撮影情報を保存しています[%s]" % fn)
        if cv2.imwrite(DATAPATH + fn + '\\top.jpg', stock_top) == False:
            logger.error("書き込みエラー")
            raise Exception()
        else:
            logger.info("[TOP]書き込み完了")
        if cv2.imwrite(DATAPATH + fn + '\\bottom.jpg', stock_bottom) == False:
            logger.error("書き込みエラー")
            raise Exception()
        else:
            logger.info("[BOTTOM]書き込み完了")
        if cv2.imwrite(DATAPATH + fn + '\\side1.jpg', stock_side1) == False:
            logger.error("書き込みエラー")
            raise Exception()
        else:
            logger.info("[SIDE1]書き込み完了")
        if cv2.imwrite(DATAPATH + fn + '\\side2.jpg', stock_side2) == False:
            logger.error("書き込みエラー")
            raise Exception()
        else:
            logger.info("[SIDE2]書き込み完了")
        if cv2.imwrite(DATAPATH + fn + '\\side3.jpg', stock_side3) == False:
            logger.error("書き込みエラー")
            raise Exception()
        else:
            logger.info("[SIDE3]書き込み完了")
        with open(DATAPATH + fn + '\\metadata.json', 'w') as f:
            f.write(json.dumps({
                'version': 1,
                'height_mm': mm,
                'timestamp': now.strftime("%Y/%m/%d %H:%M:%S")
            }, indent=4))
        barcode = barcode_recognition(
            [(stock_top, 'top'), (stock_bottom, 'bottom')])
        with open(DATAPATH + fn + '\\barcode.json', 'w') as f:
            f.write(json.dumps({
